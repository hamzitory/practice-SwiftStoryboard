...
class ViewController: UIViewController {
    ...
    //켜진 전구 이미지, 꺼진 전구 이미지, 제거된 전구 이미지
    ...
    @IBAction func btnLampON(_ sender: UIButton) {
        if(isLampOn==true) {
            //전구가 켜져있을 때 전구가 켜져있다고 알려주는 얼럿을 실행
            let lampOnAlert = UIAlertController(title: "경고", message: "현재 On 상태입니다", preferredStyle: UIAlertController.Style.alert)
            let onAction = UIAlertAction(title: "네, 알겠습니다", style: UIAlertAction.Style.default, handler: nil)
            lampOnAlert.addAction(onAction)
            present(lampOnAlert, animated: true, completion: nil)
        } else { //전구가 켜져있지 않을 경우 전구를 켬
            lampImg.image = imgOn
            isLampOn=true
        }
    }
    ...
    //전구를 제거할 것인지 묻고 켜기, 끄기, 제거 세가지 동작 중에서 선택해 실행
    @IBAction func btnLampRemove(_ sender: UIButton) {
        ...
        let offAction = UIAlertAction(title: "아니오, 끕니다(off)", style: UIAlertAction.Style.default, handler: {ACTION in self.lampImg.image = self.imgOn
            self.isLampOn=false
        })
